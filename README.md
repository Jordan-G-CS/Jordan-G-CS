## Hi there 👋

<!--
Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
## About Me
I'm a Computer Science student at Tufts University pursuing opportunities in software engineering and machine learning. My interests include observability, machine learning, and reliable software systems.

- 🎓 Studying Computer Science at Tufts Univeristy
- 💼 Currently a Break Through Tech Fellow
- 🌱 Currently learning: Python for Machine Learning, RAG, Chatbots
- 📫 Reach me: [Email](mailto:jordangasaatura2006@gmail.com) or [LinkedIn](https://www.linkedin.com/in/jordan-gasaatura/)

---

### Technical Skills

### Languages
- Python
- Go
- Java
- C++
- C

### Technologies & Tools
- Kubernetes
- Docker
- Linux
- Git
- OpenTelemetry

### Machine Learning
- scikit-learn
- pandas
- NumPy
- matplotlib
- Keras

---

### 📂 Featured Projects

_Don't have projects yet? That's ok! This is only an example_

| Project | Description | Tech |
|--------|-------------|------|
| [**Project 1**](https://github.com/Jordan-G-CS/Income_Predictor) | Built an Income Predictor to predict if a person make more or less than 50k a year using census data.| Python, scikit-learn, pandas |

---

### 📊 GitHub Highlights
- Contributions to **3+ repositories** across [open source / academic / personal projects]

### 📊 GitHub Stats: Coming Soon!

---

## Experience

### Odigos
** Software Engineer Intern ** • June 2025 - August 2025

- Led the execution and analysis of 11 end-to-end Chainsaw tests in Odigos, an open-source observability platform, uncovering multiple edge-case bugs and contributing to validate stability and instrumentation behavior 
- Created 5 internal documentation guides from scratch on test structure, edge cases, and debugging workflows, improving onboarding efficiency and knowledge sharing across teams. 
- Partnered with front end engineers to identify and resolve UI-related bugs and inconsistencies in the Odigos dashboard

---

## What I'm Currently Working On

- Convolution Neural Network to detect AI images using the CIFAKE dataset

---

### 📫 Connect with Me
- **LinkedIn:** [LinkedIn](https://www.linkedin.com/in/jordan-gasaatura/)
- **Email:** [jordangasaatura2006@gmail.com](mailto:jordangasaatura2006@gmail.com)

---

Thanks for stopping by!

